...
全局搜索面板（`Alt+Shift+F/A/T`），支持前缀路由。内置剪贴板搜索、应用启动器、多引擎翻译和 TTS 语音播放。通过插件系统可扩展。
...
| **Spotlight** | 全局搜索面板 &bull; 前缀路由（`cc`/`tt`/`aa`/`\|`） &bull; 剪贴板搜索 &bull; 应用启动器 &bull; 多引擎翻译+TTS |
| **插件** | 本地文件夹加载 &bull; JS 前端 + Rust 原生后端 &bull; 权限系统 &bull; 生命周期 Hook &bull; 右键菜单/托盘扩展 &bull; IDE Projects 插件 |
...
通过快捷键激活的全局搜索面板，支持多种模式和前缀路由：

| 模式 | 快捷键 | 前缀 | 说明 |
|------|--------|------|------|
| 剪贴板 | `Alt+Shift+F` | `cc` | 搜索剪贴板历史，回车粘贴或复制 |
| 应用启动 | `Alt+Shift+A` | `aa` | 搜索已安装应用，回车启动 |
| 翻译 | `Alt+Shift+T` | `tt` | 多引擎并行翻译，支持 TTS 语音播放 |
| IDE 项目 | - | `\|` | 搜索 VS Code、Cursor、Windsurf、Trae、Zed 的最近项目（插件） |

输入前缀加空格即可切换模式（如 `tt hello` 翻译 "hello"）。`Tab` 切换模式，`Esc` 关闭。
