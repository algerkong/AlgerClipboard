...
impl OneDriveAdapter {
    ...
    async fn ensure_token(&self) -> Result<String, String> {
        ...
        let resp = self.client.post("https://login.microsoftonline.com/consumers/oauth2/v2.0/token")
            .form(&[
    ...
    fn graph_path(&self, remote_path: &str) -> String {
        ...
        format!("https://graph.microsoft.com/v1.0/me/drive/root:/AlgerClipboard/{}:", path)
    }
}
...
impl CloudStorageAdapter for OneDriveAdapter {
    async fn test_connection(&self) -> Result<bool, String> {
        ...
        let resp = self.client.get("https://graph.microsoft.com/v1.0/me/drive")
            .bearer_auth(&token)
            .send()
            .await
    ...
    async fn upload(&self, remote_path: &str, data: &[u8]) -> Result<(), String> {
        let token = self.ensure_token().await?;
        let url = format!("{}/content", self.graph_path(remote_path).trim_end_matches(':'));
    ...
    async fn mkdir(&self, remote_path: &str) -> Result<(), String> {
        let token = self.ensure_token().await?;
        let name = remote_path.trim_start_matches('/').split('/').last().unwrap_or("AlgerClipboard");
        let resp = self.client.post("https://graph.microsoft.com/v1.0/me/drive/root/children")
            .bearer_auth(&token)
        ...
        if resp.status().is_success() || resp.status().as_u16() == 201 || resp.status().as_u16() == 409 {
            Ok(())
        } else {
