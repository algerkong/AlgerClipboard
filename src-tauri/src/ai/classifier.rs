...
pub fn classify_content(text: &str) -> ContentCategory {
    ...
    if ip_re.is_match(trimmed) {
        return ContentCategory::IpAddress;
    }

    // Phone number (various formats)
    let phone_re = regex::Regex::new(r"^[+]?[\d\s\-().]{7,20}$").unwrap();
    if phone_re.is_match(trimmed) {
        return ContentCategory::PhoneNumber;
    }

    // Color values
    let color_re =
        regex::Regex::new(r"^(#[0-9a-fA-F]{3,8}|rgba?\(\s*\d+\s*,\s*\d+\s*,\s*\d+|hsla?\(\s*\d+)")
            .unwrap();
    if color_re.is_match(trimmed) {
        return ContentCategory::Color;
    }

    // SQL query
    let sql_re =
        regex::Regex::new(r"(?i)^\s*(SELECT|INSERT|UPDATE|DELETE|CREATE|ALTER|DROP|WITH)\s")
            .unwrap();
    if sql_re.is_match(trimmed) {
        return ContentCategory::SqlQuery;
    }
    ...
    if cmd_re.is_match(trimmed) {
        return ContentCategory::CommandLine;
    }

    // Markdown (has markdown-specific syntax patterns)
    let md_indicators: &[&str] = &[
        "# ", "## ", "### ", "```", "- [ ]", "- [x]", "**", "![", "](",
    ];
    let md_count = md_indicators
        .iter()
